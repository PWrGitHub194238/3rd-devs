...
export class Neo4jService {
  ...
  async deleteNodes(nodeLabel: string): Promise<boolean> {
    const cypher = `
      MATCH (u:${nodeLabel})
      DETACH DELETE u
    `;
  ...
  async findNodeByProperty(label: string, propertyName: string, propertyValue: any): Promise<{ id: number, properties: Record<string, any> } | null> {
    ...
    async hasOutgoingRelation(fromNodeId: string, toNodeId: string, relationNmame: string): Promise<boolean> {
    const cypher = `
      OPTIONAL MATCH (a:User {id: $fromNodeId})-[r:${relationNmame}]->(b:User {id: $toNodeId})
    ...
    const record = result.records[0]?.get('connection_exists');
    return record;
  }
